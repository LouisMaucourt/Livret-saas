...
CREATE TYPE supported_language AS ENUM ('fr', 'en', 'es', 'de', 'it', 'pt');

CREATE TYPE section_type AS ENUM (
  'wifi', 'stay', 'places', 'contacts',
  'rules', 'infos', 'golden_book'
);

CREATE TYPE key_access_type AS ENUM (
  'physical_key', 'key_box', 'digicode', 'smart_lock', 'concierge'
);
...
CREATE TABLE properties (
  ...
  default_language supported_language NOT NULL DEFAULT 'fr',
  languages        supported_language[] NOT NULL DEFAULT '{fr}',
  created_at       TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE clients (
  ...
  qr_token      TEXT,
  language      supported_language NOT NULL DEFAULT 'fr',
  created_at    TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE stay_details (
  ...
  instructions_i18n       JSONB NOT NULL DEFAULT '{}',
  instructions_leave_i18n JSONB NOT NULL DEFAULT '{}',
  key_access_info         VARCHAR(100)
);
...
CREATE TABLE rules_details (
  ...
  additional_rules_i18n JSONB NOT NULL DEFAULT '{}'
);
...
CREATE INDEX idx_sections_title_i18n      ON sections USING GIN (title_i18n);
CREATE INDEX idx_places_name_i18n         ON places_details USING GIN (name_i18n);
CREATE INDEX idx_infos_title_i18n         ON infos_details USING GIN (title_i18n);
