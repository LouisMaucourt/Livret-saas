  ...
  routes: {
    ...
    "/api/auth/logout": {
      async POST() {
        ...
        headers.append("Set-Cookie", `userId=; Path=/; Max-Age=0; SameSite=Lax`);
        headers.append("Set-Cookie", `propertyId=; Path=/; Max-Age=0; SameSite=Lax`);
        headers.append("Set-Cookie", `clientId=; Path=/; Max-Age=0; SameSite=Lax`);
    ...
    "/api/auth/login": {
      ...
      async POST(req) {
        ...
        headers.append("Set-Cookie", `token=${token}; HttpOnly; Path=/; Max-Age=604800; SameSite=Lax`);
        headers.append("Set-Cookie", `userId=${user.id}; Path=/; Max-Age=604800; SameSite=Lax`);
    ...
    "/api/auth/scan": {
      ...
      `
          ...
          const headers = new Headers({ "Content-Type": "application/json" })
          headers.append("Set-Cookie", `token=${token}; HttpOnly; Path=/; Max-Age=2592000; SameSite=Lax`)
          headers.append("Set-Cookie", `clientId=${client.id}; Path=/; Max-Age=2592000; SameSite=Lax`)
...
`;
    ...
    `;
        ...
        const headers = new Headers({ "Content-Type": "application/json" });
        headers.append("Set-Cookie", `token=${sessionToken}; HttpOnly; Path=/; Max-Age=2592000; SameSite=Lax`);
        headers.append("Set-Cookie", `userId=${user.id}; Path=/; Max-Age=2592000; SameSite=Lax`);
    ...
    SELECT * FROM clients WHERE property_id = ${propertyId}
  `
  ...
  FROM sections
  LEFT JOIN infos_details 
    ON infos_details.section_id = sections.id
...
`
  ...
  `
      ...
      async DELETE(req) {
        ...
        DELETE from infos_details WHERE id = ${body.id}
        `
    ...
    "/api/wifi": {
      ...
      FROM sections
      LEFT JOIN wifi_details 
        ON wifi_details.section_id = sections.id
...
FROM sections
LEFT JOIN contacts_details
  ON contacts_details.section_id = sections.id
WHERE sections.property_id = ${bookletId}
...
FROM sections
LEFT JOIN places_details
  ON places_details.section_id = sections.id
WHERE sections.property_id = ${propertyId}
...
FROM sections
LEFT JOIN rules_details
  ON rules_details.section_id = sections.id
WHERE sections.property_id = ${propertyId}
...
FROM sections
LEFT JOIN stay_details
  ON stay_details.section_id = sections.id
WHERE sections.property_id = ${bookletId}
...
RETURNING *
    ...
    "/api/properties": {
      async GET(req) {
        if (!verifyAuth(req)) return unauthorized()
        const userId = getCookieServer(req, "userId");
    ...
    SELECT DISTINCT p.*
    FROM properties p
    LEFT JOIN property_clients pc
      ON pc.property_id = p.id
    WHERE 
      p.owner_id = ${userId}
      OR pc.user_id = ${userId}
  ...
  `
      ...
      async DELETE(req) { 
        const body = await req.json()
        const [properties] = await sql `
        DELETE FROM properties WHERE properties.id = ${body.id}
        RETURNING *
        `
        return Response.json(properties)
      }
