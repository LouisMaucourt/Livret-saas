    ...
    export const ThemePage = () => {
        ...
        useEffect(() => {
            if (data?.length) {
                setLocalData([...data].filter(s => s.type !== 'golden_book').sort((a, b) => a.sort_order - b.sort_order))
            }
        }, [data])
        console.log(data)
        ...
        const getTitle = (s: Section) => {
            const i18n = typeof s.title_i18n === 'string' ? JSON.parse(s.title_i18n) : s.title_i18n
            return i18n?.fr ?? Object.values(i18n ?? {})[0] ?? ""
        }
            ...
            <div className="flex flex-row gap-4">
                ...
                {localData.map((s, i) => (
                    ...
                    >
                        ...
                        <span className="flex-1 text-sm font-medium text-gray-800">{getTitle(s)}</span>
                        <button
