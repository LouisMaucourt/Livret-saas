  ...
  export const Places = ({ defaultCategory = PLACES_CATEGORIES[0].value }: { defaultCategory?: string }) => {
    ...
    const { id } = useParams()
    const { data, loading, error, refresh } = useApi<Places[]>(() => getPlacesApi(id))
    ...
    const handleOpen = () => {
      setCategory(view ?? PLACES_CATEGORIES[0].value)
    }
    const handleEdit = async (e: React.FormEvent, id:string, close: () => void) => { 
      e.preventDefault()
      await post(
        "/api/places",
        {
    ...
    const handleSubmit = async (e: React.FormEvent, close: () => void) => {
      e.preventDefault()
      await post("/api/places", { sectionId: data?.[0]?.section_id, name, description, address, category, phone, website }, { close: () => { resetForm(); close() }, refresh })
    }
      ...
      <>
        ...
        <div className="flex gap-2">
          {PLACES_CATEGORIES.map((cat) => (
            <button key={cat.value} onClick={() => handleView(cat.value)} className={`flex items-center gap-1.5 px-4 py-1 rounded-full text-sm border transition-colors ${view === cat.value ? "bg-violet-600 text-white border-blue-500" : "border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
              <cat.icon size={14} />
              {cat.label}
            </button>
          ))}
              ...
              {filtered.length === 0 ? (
                ...
                <InfoSection title={cat.label}>
                    {filtered.map((p) => (
                      <div
                        key={p.id}
                        className="group relative bg-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition"
                      >
                        <div className="absolute top-3 right-3 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                          ...
                          >
                            {({ close }) => (
                              <Form onSubmit={(e) => handleEdit(e, p.id, close)}>
                                <ContentInput>
              ...
              <ContentInput>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PLACES_CATEGORIES.map((c) => (
                    <button key={c.value} type="button" value={category} onClick={() => setCategory(c.value)} className={`flex items-center gap-2 p-2 rounded-lg border text-sm transition-colors ${category === c.value ? 'border-blue-500 bg-blue-50 text-blue-800' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
